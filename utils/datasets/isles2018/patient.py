...
class ISLES2018Patient:
    ...
    def load_niftis(
        ...
        for modality_dir in self.content:
            ...
            if modality in modalities:
                ...
                modality_nifti_path = glob(os.path.join(modality_dir, "*.nii"))[0]
        ...
        self.original_shape = self.ct.get_fdata().shape

    def get_data(
        self,
        modalities: List[str] = ["CT", "CBV", "CBF", "MTT", "Tmax"],
    ...
    def extract_modality_from_path(path: str) -> str:
        ...
        modality = path_shards[-2]
        return "CT" if modality == "CT" else modality[3:]

    @staticmethod
    def get_smir_id_from_path(path: str) -> str:
        path_shards = path.split("/")[-1]
        return path_shards.split(".")[-2]
