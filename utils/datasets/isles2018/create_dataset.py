...
def create_isles2018_dataset(
    ...
    for patient_path in tqdm(patient_paths, desc=f"Creating {dset_split} dataset"):
        patient = ISLES2018Patient(str(patient_path))
        patient.load_niftis()
...
def get_isles2018_train_val_test_patients(source_dset_dir: str):

    # Load the paths of all patient directories.
    train_patients_orig = glob(os.path.join(source_dset_dir, "TRAINING", "*"))
    train_patients_orig = [p for p in train_patients_orig if os.path.isdir(p)]
    train_patients_orig = [os.path.abspath(p) for p in train_patients_orig]
    train_patients_orig = np.array(sorted(train_patients_orig))

    test_patients = glob(os.path.join(source_dset_dir, "TESTING", "*"))
    test_patients = [p for p in test_patients if os.path.isdir(p)]
    test_patients = [os.path.abspath(p) for p in test_patients]
    test_patients = np.array(sorted(test_patients))

    # Split the training set into train and validation.
    np.random.seed(26)
    num_train_patients = int(len(train_patients_orig) * 0.8)
